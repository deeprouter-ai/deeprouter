...
| **F31** | 🔴 **同一秒内跑两次安装,第二次会毁掉「真原件」的备份** —— 备份名是 `<file>.bak-$DR_STAMP`,而 `DR_STAMP` 精度是**秒**;第二次的 `cp` 同名覆盖,把第一次备份的用户原始配置换成了已配置状态。manifest 层「保留首次 `original_backup`」的保护(`dr_prior`)本身工作正常,但它指向的**文件内容**已被冲掉,于是卸载还原出来的是第一次安装后的状态 | 备份名只含秒级时间戳,没有考虑同秒双跑;本机测试进程开销大(单场景 7.4s,两次 setup 天然跨秒)所以从未撞上,CI 的 Linux 上 0.14s 跑完全场景、同秒必撞 —— **PR 首次跑 CI 即红**(`TestUninstall_RestoresTheStateBeforeTheFirstInstall`) | 备份名追加进程号:`bak-$DR_STAMP-$$`(ps1 用 `$PID`),同秒也不可能同名。ubuntu 24.04 容器以非 root(=CI 身份)全量重跑通过;顺带给只读测试加 root 守卫(root 无视权限位,docker 里以 root 跑测试会假阳性,`os.Geteuid()==0` 时 skip,CI 非 root 不受影响) |
