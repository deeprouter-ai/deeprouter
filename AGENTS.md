...
- **Location**: NEW PRDs go in the **meta-repo** at `deeprouter-ai/docs/{kebab-case-name}-prd.md` — never in this repo (decided 2026-08-31: this repo is public, so a PRD committed here is published, findings and business context included; the meta-repo is also where the board and `adlc-audit.py` resolve card `parent:` paths). The pre-rule PRDs still under `docs/tasks/` migrate one at a time as they are next touched, each leaving a one-line breadcrumb (see `one-click-cli-setup-prd.md` for the pattern). This repo's own engineering references (`docs/PRD.md`, `ARCHITECTURE.md`, …) are code documentation and stay. A task PRD references the relevant product PRD; it does not duplicate it.
